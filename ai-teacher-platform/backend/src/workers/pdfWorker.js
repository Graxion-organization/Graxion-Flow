...
const processPdf = async (job) => {
  ...
  console.log(`Processing PDF for Material: ${materialId}`);
  ...
  try {
    ...
    console.error(`Failed to process material ${materialId}:`, error);
...
if (useRedis) {
  ...
  console.log('⚠️  Redis is disabled. Set USE_REDIS=true in .env to enable background PDF workers.');
}
