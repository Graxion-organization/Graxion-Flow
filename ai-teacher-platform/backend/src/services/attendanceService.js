...
const finalizeSessionAttendance = async (sessionId, batchId) => {
  ...
  for (const [studentId, data] of Object.entries(sessionData)) {
    ...
    try {
      ...
      console.error(`Error updating memory for student ${studentId}:`, err.message);
    }
  }
  ...
  try {
    ...
    console.log(`Session ${sessionId} attendance finalized and saved.`);
  } catch (err) {
    console.error('Error saving ClassSummary:', err.message);
  }
