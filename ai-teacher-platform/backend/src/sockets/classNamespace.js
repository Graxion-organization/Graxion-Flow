...
const initializeClassNamespace = (io) => {
  ...
  classNs.on('connection', (socket) => {
    ...
    console.log(`Student ${socket.id} joined class session: ${sessionId}`);
    ...
    socket.on('ask_question', async ({ question, materialId }) => {
      console.log(`Question from ${socket.studentName}: ${question}`);
    ...
    socket.on('disconnect', () => {
      console.log(`Student ${socket.id} left class session: ${sessionId}`);
      if (socket.studentId) {
