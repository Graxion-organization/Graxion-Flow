  ...
  return (
    <div className="flex flex-col gap-6 pb-6 h-auto lg:h-[calc(100vh-120px)] min-h-screen lg:min-h-0">
      ...
      {selectedAccount && (
        <div className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-bold text-lg text-slate-900 dark:text-gray-100">{selectedAccount.name || 'Unknown Profile'}</h2>
      ...
      {loadingAccounts && !selectedAccount ? (
        ...
        <div className="flex-1 flex justify-center items-center text-slate-400 dark:text-gray-500">No connected profile found. Connect a profile first.</div>
      ) : (
        ...
        <div className="p-4 border-b border-slate-200 dark:border-white/10 shrink-0 flex justify-between items-center">
          ...
          {selectedMedia && (
            <button
              onClick={async () => {
                try {
                  await api.post('/social-hub/linkedin/manual/auto-reply-post', {
              ...
              <div key={comment.id} className="space-y-2">
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-sm text-slate-800 dark:text-gray-200">@{comment.actor?.urn || "LinkedIn User"}</span>
                    <span className="text-[10px] text-slate-400 dark:text-gray-500">{new Date(comment.timestamp).toLocaleString()}</span>
                  </div>
                  <p className="text-sm text-slate-700 dark:text-gray-300">{comment.message?.text || "Comment text"}</p>
                </div>
