    ...
    <div className="flex flex-col h-full min-h-0 bg-white dark:bg-[#0b101e] rounded-2xl border border-slate-200 dark:border-white/5 overflow-hidden shadow-sm">
      ...
      <div className="flex items-center justify-between p-2 lg:p-3 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/5 shrink-0">
        <div className="flex items-center gap-2 lg:gap-3">
          <div className="w-8 h-8 rounded-lg bg-red-500 flex items-center justify-center text-white shrink-0 shadow-sm">
            <Youtube size={16} />
          </div>
          {accounts.length > 0 ? (
            <select 
            ...
            >
              {accounts.map(a => (
                <option key={a._id} value={a._id} className="text-slate-800 dark:bg-[#0f172a] dark:text-gray-100">
                  {a.channelName || 'Unknown Channel'}
                </option>
              ))}
            </select>
          ) : (
            <span className="text-sm font-bold text-slate-400 px-2">No Channels</span>
          )}
        </div>
        ...
        {selectedAccount && (
          <div className="flex items-center gap-1.5 lg:gap-2">
            ...
            <button onClick={async () => {
              ...
              className="p-1.5 lg:px-3 lg:py-1.5 bg-red-500/10 text-red-500 hover:bg-red-500/20 rounded-xl transition-colors font-bold flex items-center gap-2 text-xs"
            >
      ...
      {loadingAccounts && !selectedAccount ? (
        ...
        <div className="flex-1 flex justify-center items-center text-slate-400 dark:text-gray-500 text-sm font-medium">No connected channels found. Connect a channel first.</div>
      ) : (
          ...
          <div className={`w-full lg:w-[320px] xl:w-[350px] flex-col border-r border-slate-200 dark:border-white/5 bg-slate-50/50 dark:bg-black/20 shrink-0 h-full ${selectedMedia ? 'hidden lg:flex' : 'flex'}`}>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              {loadingMedia ? (
                ...
                <div className="text-center text-slate-400 dark:text-gray-500 text-xs py-8">No videos found.</div>
              ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <div className="flex items-center justify-between mb-0.5">
                        <span className="text-[10px] font-bold text-red-500 uppercase tracking-wider">{media.media_type || 'VIDEO'}</span>
                      </div>
                      <div className="text-sm font-semibold text-slate-800 dark:text-gray-200 line-clamp-1 leading-tight">
                        {media.snippet?.title || 'No Title'}
                      </div>
                    </div>
                  </button>
                ))
              ...
              <>
                ...
                <div className="p-3 border-b border-slate-200 dark:border-white/5 shrink-0 flex justify-between items-center bg-white/90 dark:bg-[#0b101e]/90 backdrop-blur-md z-10 absolute top-0 left-0 right-0 lg:hidden">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <img src={selectedMedia.snippet?.thumbnails?.medium?.url || selectedMedia.snippet?.thumbnails?.high?.url || 'https://via.placeholder.com/150'} className="w-8 h-8 rounded-lg object-cover" />
                      <div className="flex flex-col">
                        <span className="font-bold text-sm text-slate-800 dark:text-gray-100 leading-tight">Video Comments</span>
                ...
                {autoReplyProgress && (
                  <div className="absolute top-[57px] lg:top-0 left-0 right-0 bg-red-500/10 border-b border-red-500/20 p-2 z-10 backdrop-blur-md">
                    ...
                    <div className="w-full bg-black/10 rounded-full h-1 overflow-hidden">
                      <div className="bg-red-500 h-1 transition-all duration-300" style={{ width: `${autoReplyProgress.total > 0 ? (autoReplyProgress.processed / autoReplyProgress.total) * 100 : 0}%` }}></div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 lg:p-6 space-y-6 pt-20 lg:pt-6 pb-20">
                  {loadingComments ? (
                    ...
                    <div className="text-center text-slate-400 dark:text-gray-500 text-xs py-8">No comments on this video yet.</div>
                  ) : (
                          ...
                          <div className="flex gap-3">
                            ...
                            <div className="flex flex-col items-start max-w-[85%]">
                              <div className="flex items-baseline gap-2 mb-1">
                                <span className="font-bold text-xs text-slate-700 dark:text-gray-300">{username}</span>
                              ...
                              <div key={reply.id || reply.etag} className="flex gap-3 justify-end pl-12">
                                <div className="flex flex-col items-end max-w-[85%]">
                                  <div className="flex items-baseline gap-2 mb-1">
                                    ...
                                    <span className="font-bold text-xs text-red-500">{rUser}</span>
                                  </div>
                                  <div className="px-4 py-2.5 rounded-2xl rounded-tr-sm text-sm bg-red-500 text-white shadow-sm" dangerouslySetInnerHTML={{ __html: rText }} />
                                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 bg-white/90 dark:bg-[#0b101e]/90 backdrop-blur-xl border-t border-slate-200 dark:border-white/5 shrink-0 z-20">
                  ...
                  <div className="flex items-end gap-2">
                    <div className="flex-1 bg-slate-100 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-2xl px-4 py-1.5 focus-within:border-red-500 dark:focus-within:border-red-500 transition-colors">
                      <textarea
                        rows={1}
                        placeholder={selectedComment ? "Type a reply..." : "Comment on video..."}
                        value={messageText}
                        onChange={e => {
                    ...
                    <button
                      ...
                      className="w-10 h-10 shrink-0 bg-red-500 hover:bg-red-600 disabled:bg-slate-300 dark:disabled:bg-slate-700 text-white rounded-full flex items-center justify-center transition-all shadow-sm mb-1"
                    >
        ...
        <div className="fixed inset-0 z-[200] lg:hidden flex items-end justify-center bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setMobileMenuMedia(null)}>
          <div className="bg-white dark:bg-[#0f172a] w-full rounded-t-3xl p-5 flex flex-col gap-3 shadow-2xl translate-y-0 animate-slide-up" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-3 mb-2 px-1">
              <img 
                src={mobileMenuMedia.snippet?.thumbnails?.medium?.url || mobileMenuMedia.snippet?.thumbnails?.high?.url || 'https://via.placeholder.com/150'} 
                className="w-10 h-10 object-cover rounded-xl shrink-0 bg-slate-100 dark:bg-black/40"
              />
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-bold text-slate-800 dark:text-gray-200 truncate">{mobileMenuMedia.snippet?.title || 'No title'}</h3>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Video Options</span>
              </div>
            </div>
