  ...
  return (
    <div className="h-[calc(100vh-120px)] flex flex-col gap-6 pb-6">
      ...
      {selectedAccount && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-bold text-lg text-gray-100">{selectedAccount.channelName || 'Unknown Channel'}</h2>
      ...
      {loadingAccounts && !selectedAccount ? (
        ...
        <div className="flex-1 flex justify-center items-center text-gray-500">No connected channels found. Connect a channel first.</div>
      ) : (
              ...
              <div key={comment.id} className="space-y-2">
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-sm text-gray-200">@{comment.snippet?.topLevelComment?.snippet?.authorDisplayName || comment.authorDisplayName || "Unknown"}</span>
                    <span className="text-[10px] text-gray-500">{new Date(comment.timestamp).toLocaleString()}</span>
                  </div>
                  <p className="text-sm text-gray-300">{comment.snippet?.topLevelComment?.snippet?.textDisplay || comment.text || ""}</p>
                </div>
                ...
                {comment.replies && comment.replies.data && comment.replies.data.map(reply => (
                  <div key={reply.id} className="ml-8 p-3 rounded-xl bg-black/40 border border-white/5">
                    ...
                    <p className="text-xs text-gray-300">{reply.snippet?.textDisplay || reply.text || ""}</p>
                  </div>
                ))}
              </div>
            ))
