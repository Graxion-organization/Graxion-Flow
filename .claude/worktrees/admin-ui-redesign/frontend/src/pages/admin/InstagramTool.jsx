...
export default function InstagramTool() {
  ...
  const fetchAccounts = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${baseURL}/admin/instagram/accounts`, {
  ...
  const fetchMedia = async (account) => {
    ...
    try {
      ...
      axios.get(`${baseURL}/admin/instagram/${account._id}/stats`, {
        withCredentials: true
      }).then(res => {
      ...
      const res = await axios.get(`${baseURL}/admin/instagram/${account._id}/media`, {
        withCredentials: true
      });
  ...
  const fetchComments = async (media) => {
    ...
    try {
      // JWT via cookies
      const baseURL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api';
      const res = await axios.get(`${baseURL}/admin/instagram/${selectedAccount._id}/media/${media.id}/comments`, {
        withCredentials: true
      });
  ...
  const handleSendComment = async () => {
    ...
    try {
      ...
      await axios.post(`${baseURL}/admin/instagram/comment`, {
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex gap-6 pb-6">
      ...
      {/* COLUMN 1: Accounts */}
      <div className="w-1/4 bg-white/5 border border-white/10 rounded-3xl flex flex-col overflow-hidden shrink-0">
            ...
            <button 
              onClick={async () => {
                try {
                  // JWT via cookies
                  const baseURL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api';
                  await axios.post(`${baseURL}/admin/instagram/trigger-worker`, {}, {
                    withCredentials: true
                  });
              ...
              className="p-1.5 bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 rounded-lg transition-colors flex items-center gap-1 text-xs px-2"
            >
              <RefreshCw className="w-3 h-3" /> Sync DMs
            </button>
            <button onClick={fetchAccounts} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
          {loadingAccounts ? (
            ...
            <div className="text-center text-gray-500 text-sm py-8">No connected accounts found.</div>
          ) : (
            accounts.map(acc => (
              <button
                key={acc._id}
                onClick={() => fetchMedia(acc)}
              ...
              >
                <div className="font-bold text-sm text-gray-200">@{acc.igUsername || 'Unknown'}</div>
                <div className="text-xs text-gray-500 flex items-center gap-1 mt-1">
                  <UserIcon className="w-3 h-3" />
                  {acc.user?.name || 'No User'}
                </div>
              </button>
            ))
          )}
            ...
            <button
              onClick={async () => {
                try {
                  // JWT via cookies
                  const baseURL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api';
                  await axios.post(`${baseURL}/admin/instagram/auto-reply-post`, {
                    accountId: selectedAccount._id,
